...
public:
	...
	QPBO(int node_num_max, long long edge_num_max, void (*err_function)(const char *) = NULL);
	// Copy constructor
	QPBO(QPBO<REAL>& q);
	...
	long long GetMaxEdgeNum(); // returns the number of edges for which the memory is allocated. 
	void SetMaxEdgeNum(long long num); // If num > edge_num_max then memory for edges is reallocated. Important for Probe() with option=1,2.
...
private:
	...
	long long	arc_shift; // = 2*edge_num_max*sizeof(Arc)
	...
	void reallocate_arcs(long long arc_num_max_new);
...
template <typename REAL> 
	inline long long QPBO<REAL>::GetMaxEdgeNum() 
{
	return (long long)(arc_max[0]-arcs[0])/2;
}
